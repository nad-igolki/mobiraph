...
def train_7mer(fasta_path: str, checkpoint_dir: str, metadata_file: str, processes: int | None = None):
    process_k(7, fasta_path, checkpoint_dir, processes=processes)

    EMBEDDINGS_PATH = Path(checkpoint_dir) / "7.csv"
    METADATA_PATH = metadata_file

    OUTPUTS_DIR = Path(checkpoint_dir) / "7mer" / "models"
    OUTPUTS_DIR.mkdir(parents=True, exist_ok=True)

    OUTPUTS_IMAGES_DIR = Path(checkpoint_dir) / "7mer"
    OUTPUTS_IMAGES_DIR.mkdir(parents=True, exist_ok=True)
    OUTPUTS_LOGITS_RESULTS_DIR = Path(checkpoint_dir) / "7mer" / "logits"
...
def test_7mer(fasta_path: str, checkpoint_dir: str, models_path: str, processes: int | None = None):
    process_k(7, fasta_path, checkpoint_dir, processes=processes)
    EMBEDDINGS_PATH = Path(checkpoint_dir) / "7.csv"
    METADATA_PATH = None
    TRAIN_IDS_PATH = None

    OUTPUTS_DIR = Path(models_path) / "7mer" / "models"
