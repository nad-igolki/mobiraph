...
for name in insect_names:
    class_file = os.path.join(directory, f'{name}_sv_te_class.txt')
    partition_file = os.path.join(directory, f'{name}_node_partition.txt')
    fasta_file = os.path.join(directory, f'{name}_seq.fasta')
    ...
    if name_to_class[name] is not None and 'sv' in name_to_class[name]:
        ...
        output_fasta = os.path.join(directory, f'{name}_target_sequences.fasta')
        ...
        output_labels = os.path.join(directory, f'{name}_target_labels.txt')
        with open(output_labels, 'w') as f:
            for [node_name, type_value] in names_class_to_take:
                f.write(type_value + '\n')

        print(f'Список сохранён в файлы: {output_fasta}, {output_labels}')
