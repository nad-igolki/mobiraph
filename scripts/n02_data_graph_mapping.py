...
for insect_name in tqdm(insect_names, desc="Processing insects"):
    class_file = os.path.join(config.DIR_GRAPH_PARTITION, f'{insect_name}_sv_te_class.txt')
    partition_file = os.path.join(config.DIR_GRAPH_PARTITION, f'{insect_name}_node_partition.txt')
    fasta_file = os.path.join(config.DIR_GRAPH_PARTITION, f'{insect_name}_seq.fasta')
    ...
    for name_value in name_to_sequence[insect_name].keys():
        ...
        try:
            ...
            if class_df is not None:
                filtered_class_df = class_df.loc[class_df['sv'] == name_value, 'type']
